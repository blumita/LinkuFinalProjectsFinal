<template>
  <div>
    ...
    <div class="text-center">
      <h2 class="text-lg font-bold text-gray-700">وضعیت حساب</h2>
      <p class="text-sm text-gray-500">
        برای گرفتن بیشترین نتیجه از کارت دیجیتال لینکو، این چک‌لیست را کامل کنید
      </p>

      <div class="w-full h-3 rounded-full border border-gray-300 overflow-hidden mt-4">
...
const actionHandlers = computed(() => ({
  profile: {
    ...
    //disabled: !defaultCard.value?.avatar
  },
  link: {
    status: defaultCard.value?.linksDataList ? 'done' : 'pending',
    //disabled: !defaultCard.value?.linksDataList
  },
  map: {
    status: 'pending',
    disabled: true
