<template>
  <div>
    <h2 class="text-xl font-bold mb-6 text-right flex items-center gap-2 text-primary">
      <i class="ti ti-egg text-2xl text-primary"></i>
      تخم‌مرغ شانس - شرکت‌کنندگان
    </h2>
    ...
    <div v-if="loading" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      >
        ...
        <div class="absolute top-3 left-3 w-8 h-6 bg-primary rounded"></div>
      </div>
    </div>
    ...
    <div v-else-if="people.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      >
        <div class="space-y-2 text-sm text-primary text-right">
          ...
          <div v-if="item.ip" class="flex items-center gap-1 opacity-60">
    ...
    <div v-else class="flex flex-col items-center justify-center py-16 text-center">
      <div class="w-24 h-24 rounded-full bg-secondary flex items-center justify-center mb-4">
        <i class="ti ti-egg text-5xl text-primary opacity-30"></i>
      </div>
      ...
      <p class="text-sm text-primary opacity-60 max-w-sm">
        هنوز هیچ کاربری در تخم‌مرغ شانس شرکت نکرده است.
      </p>
    </div>
...
export default {
  ...
  methods: {
    async fetchPeopleLuckyEgg() {
      this.loading = true
      try {
        const response = await this.$axios.get(`club/${this.cardId}/luckyEgg/allResults`)
        const luckyEggs = response.data.data || []

        // ✅ داده‌های API رو نگاشت کن به people
        this.people = luckyEggs.map(item => ({
          name: item.name || '',
          phone: item.phone || '',
    ...
    async readAllResults() {
      try {
        const response = await this.$axios.get(`club/${this.cardId}/luckyEgg/readAll`)
    ...
    exportToExcel() {
      ...
      XLSX.utils.book_append_sheet(book, sheet, 'تخم‌مرغ شانس')
      XLSX.writeFile(book, 'LuckyEgg.xlsx')
    },
...
export async function fetchPeopleLuckyEgg(cardId, axiosInstance) {
  try {
