<template>
  <svg
   id="uuid-5b552e3d-422e-49ea-9582-676a635a0255"
   viewBox="0 0 217.3294 219.10001"
   version="1.1"
   :width="size"
   :height="size"
   xmlns="http://www.w3.org/2000/svg"
   xmlns:svg="http://www.w3.org/2000/svg">
  <defs
     id="defs5">
    <clipPath
       id="uuid-a7ebb6b1-9d17-4b11-9838-26457a10fe41">
      <rect
         x="376.10001"
         y="22.299999"
         width="219.10001"
         height="219.10001"
         rx="63.400002"
         ry="63.400002"
         fill="none"
         id="rect2" />
    </clipPath>
  </defs>
  <g
     clip-path="url(#uuid-a7ebb6b1-9d17-4b11-9838-26457a10fe41)"
     id="g19"
     transform="translate(-376.975,-22.299999)">
    <g
       id="g17">
      <path
         d="m 594.1,131 c 0,22.4 0,44.8 0,67.2 0,17.2 -12.5,34.3 -27.9,41.2 -7.7,3.4 -15.7,4.5 -23.9,4.5 -38,0 -76.1,0 -114.1,0 -12.7,0 -24.3,-3.2 -34.1,-11.4 -10.1,-8.5 -16.8,-19.3 -16.9,-32.7 -0.3,-46.1 -0.3,-92.3 0,-138.4 0.1,-21.9 22.4,-44 45.2,-43.8 42,0.4 84,0 126,0.2 18.4,0 31.4,9.4 40.3,25.2 4.1,7.4 5.7,15.1 5.6,23.4 -0.2,21.5 0,42.9 0,64.4 z m -202.3,-0.6 c 0.6,50.4 40.4,93.2 93.5,93 51.2,-0.1 92.6,-41.3 92.8,-92.6 0.2,-52.7 -41.5,-93.8 -93.4,-93.6 -51.3,0.2 -92,40.7 -92.9,93.1 z"
         :fill="props.color ? props.color : '#1d1816'"
         id="path7" />
      <!-- دایره سبز فقط در حالت پیش‌فرض نمایش داده می‌شود -->
      <path
         v-if="!props.color"
         d="m 391.8,130.3 c 0.9,-52.5 41.6,-92.9 92.9,-93.1 51.9,-0.2 93.6,40.9 93.4,93.6 -0.2,51.3 -41.5,92.4 -92.8,92.6 -53.1,0.1 -92.9,-42.7 -93.5,-93 z M 421.2,100 c 0,6.2 4.9,10.7 10.5,9.1 17.3,-4.8 34.9,-5.5 52.8,-4.5 19.5,1.1 37.7,5.8 55.1,14.4 5.9,2.9 10,1.9 12.8,-3.3 2.4,-4.5 0.9,-8.5 -4.8,-11.8 -13,-7.5 -27.1,-12 -41.9,-14.5 -26.3,-4.5 -52.4,-4.5 -78.3,2.8 -4.2,1.2 -6.5,3.4 -6.1,7.7 z m 48.5,18.3 c -12.3,-0.5 -24.2,2 -35.9,5.2 -5.9,1.6 -7.5,4.7 -5.9,9.8 1.7,5.4 6,4.7 10.1,3.9 6.9,-1.3 13.8,-3.5 20.8,-3.9 24.2,-1.4 47.5,2.3 69,14.5 4.5,2.6 9.6,1.7 11.4,-1.6 2,-3.5 0.5,-8 -4,-10.7 C 515,123.4 492.9,119 469.7,118.3 Z m -1.7,30.6 c -9.3,0.5 -21,1 -32.4,4.5 -4,1.2 -5.1,3.8 -4.1,7.2 0.9,3.2 3.6,4.7 7,4 13.9,-3 27.8,-5.3 42.2,-3.5 13.1,1.6 25.5,4.7 37,11.1 3.3,1.9 7.3,3.4 9.9,-0.9 2.9,-4.7 -0.5,-7 -4.2,-9.2 -16.3,-9.7 -34.1,-13.4 -55.4,-13.4 z"
         fill="#57ba5e"
         id="path9" />
      <!-- خطوط موسیقی فقط در حالت رنگی نمایش داده می‌شوند -->
      <g v-if="props.color">
        <path
           d="m 421.2,100 c -0.4,-4.3 2,-6.5 6.1,-7.7 25.9,-7.3 52,-7.3 78.3,-2.8 14.8,2.5 28.8,7 41.9,14.5 5.7,3.3 7.3,7.3 4.8,11.8 -2.8,5.1 -6.9,6.2 -12.8,3.3 -17.4,-8.6 -35.6,-13.3 -55.1,-14.4 -17.9,-1 -35.4,-0.3 -52.8,4.5 -5.6,1.6 -10.6,-2.9 -10.5,-9.1 z"
           :fill="props.color"
           id="path11" />
        <path
           d="m 469.7,118.3 c 23.2,0.6 45.3,5.1 65.5,17.2 4.6,2.7 6.1,7.2 4,10.7 -1.9,3.3 -6.9,4.1 -11.4,1.6 -21.5,-12.2 -44.8,-15.9 -69,-14.5 -7,0.4 -13.9,2.6 -20.8,3.9 -4.1,0.8 -8.4,1.5 -10.1,-3.9 -1.6,-5.1 0,-8.2 5.9,-9.8 11.7,-3.2 23.6,-5.7 35.9,-5.2 z"
           :fill="props.color"
           id="path13" />
        <path
           d="m 468,148.9 c 21.3,0 39.1,3.7 55.4,13.4 3.7,2.2 7,4.5 4.2,9.2 -2.6,4.3 -6.6,2.7 -9.9,0.9 -11.5,-6.5 -23.9,-9.5 -37,-11.1 -14.5,-1.8 -28.3,0.5 -42.2,3.5 -3.4,0.7 -6.1,-0.8 -7,-4 -1,-3.4 0.1,-6 4.1,-7.2 11.4,-3.4 23.1,-4 32.4,-4.5 z"
           :fill="props.color"
           id="path15" />
      </g>
    </g>
  </g>
</svg>
</template>

<script setup lang="ts">
import { computed } from 'vue'

interface Props {
  size?: number | string
  filled?: boolean
  color?: string
}

const props = withDefaults(defineProps<Props>(), {
  size: 24,
  filled: false,
  color: ''
})

// تابع برای تشخیص رنگ سفید
const isWhiteColor = (color: string): boolean => {
  if (!color || color === "transparent") return false
  
  // حذف # از ابتدای رنگ hex
  const hex = color.replace("#", "").toLowerCase()
  
  // بررسی اینکه رنگ سفید باشد
  return hex === "ffffff" || hex === "fff" || color.toLowerCase() === "white"
}

// تعیین رنگ متن بر اساس رنگ پس‌زمینه
const textColor = computed(() => {
  if (!props.color) return '#000000' // مقدار پیش‌فرض برای حالت عادی
  return isWhiteColor(props.color) ? '#000000' : '#ffffff'
})
</script>