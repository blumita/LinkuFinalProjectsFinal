...
import type {MapBlock, Placeholder} from '@/types/data'
...
const locationSelected = ref(
  !!(props.link?.latitude && props.link?.longitude && 
     props.link.latitude !== 35.6992 && props.link.longitude !== 51.3890)
)
const selectedAddressText = ref(props.link?.address || '')
...
const iconComponent = computed(() => {
  ...
  // اگر string است، آن را به عنوان component name استفاده کن
  if (typeof iconData.value === 'string') {
    return getIconComponent(iconData.value);
  }

  // اگر object است و type آن component است
  if (typeof iconData.value === 'object' && iconData.value.type === 'component') {
    return getIconComponent(iconData.value.path || iconData.value.url || '');
  }
...
const iconColor = computed(() => {
  ...
  // در غیر این صورت، مشکی برگردان
  return '#000000'
})

const isIconFilled = computed(() => {
  return true // همیشه filled باشد
})
...
function setLocation(lat: number, lng: number, cityName: string) {
  form.latitude = lat
  form.longitude = lng
  if (!form.title) {
    form.title = `موقعیت ${cityName}`
  }
}
...
function handleMapConfirm(data: { latitude: number; longitude: number; address?: string }) {
  ...
  // اگر کاربر عنوان ننوشته، عنوان پیش‌فرض "نقشه" باشد
  if (!form.title) {
    form.title = 'نقشه'
  }
  // باتم‌شیت انتخاب نوع نمایش را باز کن
  showMapPreviewModal.value = true
}

function openMapSelector() {
  // برای ویرایش، locationSelected را false کن تا MapboxComponent دوباره رندر شود
  // و سپس نقشه فول‌اسکرین را باز کن
  locationSelected.value = false
  nextTick(() => {
    nextTick(() => {
      if (mapboxRef.value) {
        mapboxRef.value.openFullscreenMap()
      }
    })
  })
}
...
async function submitForm() {
  ...
  if (!locationSelected.value) {
    // اگر موقعیت انتخاب نشده، باتم‌شیت باز شود
    showMapPreviewModal.value = true
    return
  }
  emit('submit', {...form})
