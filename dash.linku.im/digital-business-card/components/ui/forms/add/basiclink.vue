...
function showPrefix() {
  // Only show prefix for username-based actions and number/call/facetime
...
function submitForm() {
  ...
  let value =cleanValue //form.value

  if (isUsernameAction() && form.baseUrl && form.username) {
    value = form.username
  } else if (isPrefixAction() && form.value) {
    // Remove any accidental prefix from value
...
function isPrefixAction() {
  // Actions that have a protocol prefix and should show prefix span (email, call, number, facetime)
...
function getInputLabel() {
  ...
  // If placeholder.title exists, show it
  if (form.placeholder?.title) {
    return form.placeholder.title;
  }
  // If name or title exists in the item itself
  if (form.name) {
    return form.name;
  }
  if (form.title) {
...
function getInputPlaceholder() {
  ...
  // If placeholder.link exists, show it
  if (form.placeholder?.link) {
    return form.placeholder.link;
  }
  // If baseUrl exists and action is username-based, only ask for username
  const usernameActions = [
  ...
  // If baseUrl exists and action is not username-based, create appropriate test value with baseUrl
  if (form.baseUrl) {
...
function _handleNumberInput(e) {
  // Only Latin numbers and + are allowed
  e.target.value = e.target.value.replace(/[^0-9+]/g, '');
  form.username = e.target.value;
}

// Only simple email allowed
function _handleEmailInput(e) {
  // Automatically remove mailto: from beginning of value
  if (e.target.value.startsWith('mailto:')) {
    e.target.value = e.target.value.replace(/^mailto:/i, '');
  }
  // Only valid email is allowed (invalid characters are not removed but type=email validates)
  form.value = e.target.value;
}

function _getRawValue() {
  // For actions that have prefixes, return raw value without prefix
  if (["email", "call", "number", "facetime"].includes(form.action) && form.value) {
    let prefix = '';
    if (form.action === 'email') prefix = 'mailto:';
    if (form.action === 'call') prefix = 'tel:';
    if (form.action === 'number') prefix = 'sms:';
    if (form.action === 'facetime') prefix = 'facetime:';
    if (form.value.startsWith(prefix)) {
      return form.value.replace(prefix, '');
    }
  }
  return form.value || '';
}
