  ...
  <div class="bg-background rounded-xl space-y-6 w-full text-center max-h-[90vh] overflow-y-auto p-4">
    ...
    <div class="flex flex-row items-center gap-4 mb-4 w-full justify-start rtl:justify-start">
      <div class="relative w-20 h-20 flex-shrink-0 order-1 flex items-center justify-center">
        ...
        <component :is="iconComponent" v-else-if="iconComponent" class="w-full h-full rounded-xl bg-muted text-muted-foreground p-2 cursor-pointer" @click="fileInputRef?.click()" />
        <div v-else class="w-full h-full rounded-xl bg-muted flex items-center justify-center text-muted-foreground text-2xl cursor-pointer" @click="fileInputRef?.click()">
          <i class="ti ti-award" />
        </div>
        <input
          ref="fileInputRef"
    ...
    <div class="text-right space-y-1 mb-6">
      <div class="flex items-center gap-2 mb-2">
        ...
        >
          <span :class="['bg-white w-4 h-4 rounded-full shadow transform transition', form.showDescription ? '-translate-x-4' : '']" />
        </button>
