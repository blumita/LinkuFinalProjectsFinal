<template>
  <div class="bg-background rounded-xl space-y-6 w-full max-h-[90vh] overflow-y-auto p-4">
    ...
    <!-- آپلود آیکون -->
    <div class="flex flex-row items-center gap-4 mb-4 w-full justify-start rtl:justify-start">
      <div class="relative w-20 h-20 flex-shrink-0 order-1 flex items-center justify-center">
        ...
        <img v-else src="/images/lucky-dice.png" class="w-full h-full rounded-xl bg-muted object-contain p-2 cursor-pointer" @click="fileInputRef?.click()" >
        <input
          ref="fileInputRef"
          type="file"
      ...
      <p class="text-sm text-primary font-medium cursor-pointer order-2 flex-1 text-right" @click="fileInputRef?.click()">برای تغییر آیکون کلیک کنید</p>
    </div>
    ...
    <div class="text-right space-y-1 mb-4">
      ...
      <input
        v-model="form.title"
        type="text"
        placeholder="عنوان تاس شانس را وارد کنید"
    ...
    <div class="text-right space-y-1 mb-6">
      <div class="flex items-center gap-2 mb-2">
        ...
        >
          <span
            class="absolute left-0 top-0 w-10 h-6 rounded-full transition-colors duration-200"
            :style="{ backgroundColor: form.showDescription ? iconColor : 'rgba(156, 163, 175, 0.3)' }"
          />
          <span
            class="absolute left-0 top-0 w-6 h-6 bg-white rounded-full shadow transform transition-transform duration-200"
            :class="form.showDescription ? 'translate-x-4' : ''"
          />
        </button>
      ...
      <transition name="fade">
        <textarea
          ...
          placeholder="توضیحات مربوط به تاس شانس را وارد کنید"
...
const form = reactive({
  ...
  icon: props.link?.icon || { name: 'luckydice', library: 'custom', type: 'component' },
