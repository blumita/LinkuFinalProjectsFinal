      ...
      if (unref(rewardStore).isLoading) {
        ...
        if (filteredDiscounts.value.length === 0) {
          ...
          ssrRenderList(filteredDiscounts.value, (discount) => {
            ...
            if (discount.banner) {
              _push(`<div class="relative w-full h-32 lg:h-40 overflow-hidden"><img${ssrRenderAttr("src", discount.banner)}${ssrRenderAttr("alt", discount.title)} class="w-full h-full object-cover"> <div class="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent"></div> <div class="absolute top-3 right-3 bg-primary/95 backdrop-blur-sm text-white px-3 py-1.5 rounded-full text-xs font-bold shadow-lg"><i class="ti ti-discount text-sm"></i> ${ssrInterpolate(discount.value)}${ssrInterpolate(discount.type === "percentage" ? "%" : " \u0631\u06CC\u0627\u0644")}</div> <span class="${ssrRenderClass([
              ...
              ])}">${ssrInterpolate(getDiscountStatusText(discount.status))}</span></div>`);
            } else {
              _push(`<!---->`);
            }
            _push(` <div class="p-5 lg:p-6"><div class="flex items-start justify-between mb-3"><div class="flex-1"><div class="flex items-center gap-2 mb-2 flex-wrap"><h4 class="font-medium text-foreground lg:text-base">${ssrInterpolate(discount.title)}</h4> `);
            if (!discount.banner) {
              _push(`<span class="${ssrRenderClass([
                "text-xs px-2.5 py-1 rounded-full font-medium",
                getDiscountStatusClass(discount.status)
              ])}">${ssrInterpolate(getDiscountStatusText(discount.status))}</span>`);
            } else {
              _push(`<!---->`);
            }
            _push(`</div> <p class="text-sm text-muted-foreground mb-4 line-clamp-2">${ssrInterpolate(discount.description)}</p> `);
            ...
            _push(` <div class="flex items-center gap-4 text-xs text-muted-foreground mb-4 lg:mb-0">`);
            if (!discount.banner) {
              _push(`<span class="flex items-center gap-1.5"><i class="ti ti-percentage"></i> ${ssrInterpolate(discount.value)}${ssrInterpolate(discount.type === "percentage" ? "%" : " \u0631\u06CC\u0627\u0644")}</span>`);
            } else {
              _push(`<!---->`);
            }
            _push(` <span class="flex items-center gap-1.5"><i class="ti ti-calendar"></i> ${ssrInterpolate(formatDate(discount.expiryDate))}</span></div></div> <div class="w-14 h-14 lg:w-16 lg:h-16 bg-gradient-to-br from-primary/10 to-primary/5 rounded-xl flex items-center justify-center border border-primary/20 overflow-hidden flex-shrink-0">`);
            if (discount.image) {
              _push(`<img${ssrRenderAttr("src", discount.image)}${ssrRenderAttr("alt", discount.title)} class="w-full h-full object-cover">`);
            } else {
              _push(`<i class="ti ti-discount text-primary text-2xl lg:text-3xl"></i>`);
            }
            _push(`</div></div> <div class="pt-3 mt-auto border-t border-border">`);
...
//# sourceMappingURL=rewards-CSdsUSWD.mjs.map
